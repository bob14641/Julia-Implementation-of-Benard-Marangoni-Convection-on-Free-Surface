function stream(nnx,nny,nnode,ndir,vpi,solidnode,solidstr,f1,f2,t1,t2,temp,thot,tcold,tmid,phi,phip,phim,invphip,omegat,uwxr,uwxl,uwxb,uwxt,uwyr,uwyl,uwyb,uwyt,cs2,dt,fbx,fby,sigt,sigtt,tauf,nu,q_o,lq,xo,wf4)

#Streaming step for all bulk fluid nodes
for x = 1 : nnx
    for y = 1 : nny 
        for d = 1 : ndir
            ys = y - vpi[d,1]
...
#Streaming step for all boundary nodes. 
for x = 1 : nnx
    for y = 1 : nny
        in = indicies_scalar(nnx,x,y)
        ...
        elseif (solidnode[in] == 3) #Top Wall 
            ...
            qq_x = -q_o * exp(-((y - xo) * (y - xo)) / (2.0 * lq * lq))

            tlct = (ts0 + ts1 + ts3 + 2.0 * (ts2 + ts5 + ts6)) - (cs2 / omegat) * qq_x
            tlctp = (tsp0 + tsp1 + tsp3 + 2.0 * (tsp2 + tsp5 + tsp6)) - (cs2 / omegat) * qq_x
            tlctm = (tsm0 + tsm1 + tsm3 + 2.0 * (tsm2 + tsm5 + tsm6)) - (cs2 / omegat) * qq_x
            ...
            in_jm = indicies_scalar(nnx,xm,y)
            in_jmm = indicies_scalar(nnx,xmm,y)

            in_ip_jm = indicies_scalar(nnx,xm,yp)
            in_ip_jmm = indicies_scalar(nnx,xmm,yp)

            in_im_jm = indicies_scalar(nnx,xm,ym)
            in_im_jmm = indicies_scalar(nnx,xmm,ym)

            ui_ym = ui[in_jm]
            ui_ymm = ui[in_jmm]
 
            ui_xp_ym = ui[in_ip_jm]
            ui_xp_ymm = ui[in_ip_jmm]

            ui_xm_ym = ui[in_im_jm]
            ui_xm_ymm = ui[in_im_jmm]
            ...
            LAM = (sigt + 2.0 * sigtt * (tlct - tmid)) * dtx
            
            rwt = (fs0 + fs1 + fs3 + 2.0 * (fs2 + fs5 + fs6) + 0.5 * fby * dt)

            uwxtg = (2.0 / (3.0 * rwt * nu)) * LAM + (4.0 / 3.0) * ui_ym - (1.0 / 3.0) * ui_ymm
            uwxtp = (2.0 / (3.0 * rwt * nu)) * LAM + (4.0 / 3.0) * ui_xp_ym - (1.0 / 3.0) * ui_xp_ymm
            uwxtm = (2.0 / (3.0 * rwt * nu)) * LAM + (4.0 / 3.0) * ui_xm_ym - (1.0 / 3.0) * ui_xm_ymm

            diui = 0.5 * (uwxtp - uwxtm)

            C10t = rwt * (uwxtg) - (fs1 - fs3 + fs5 - fs6) - 0.5 * fbx * dt
            C01t = - (fs2 + fs5 + fs6) - 0.5 * fby * dt
            C2002t = (2.0 * phi * cs2 * rwt) + (phip * rwt * uwxtg * uwxtg) - (phip * fbx * uwxtg * dt) - (2.0 * (1.0 - cs2) * rwt * diui / wf4) - (phi * (fs1 + fs2 + fs3 + 2.0 * fs5 + 2.0 * fs6) + (fs1 - fs2 + fs3))
        ...
        elseif (solidnode[in] == 5) #Bottom-Left Corner
            ...
            dtx = (ts0 + 2.0 * ts3 + 2.0 * ts4 + 4.0 * ts7 - twbl) / (cs2 / omegat)
        ...
        elseif (solidnode[in] == 6) #Bottom-Right Corner
            ...
            dtx = (twbr - (ts0 + 2.0 * ts1 + 2.0 * ts4 + 4.0 * ts8)) / (cs2 / omegat)
        ...
        elseif (solidnode[in] == 7) # Top-Left Corner
            ...
            dtx = (ts0 + 2.0 * ts2 + 2.0 * ts3 + 4.0 * ts6 - twtl) / (cs2 / omegat)
        ...
        elseif (solidnode[in] == 8) # Top-Right Corner
            ...
            dtx =  (twtr - (ts0 + 2.0 * ts1 + 2.0 * ts2 + 4.0 * ts5)) / (cs2 / omegat)
