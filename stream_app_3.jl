function stream(nnx,nny,nnode,ndir,vpi,solidnode,solidstr,f1,f2,t1,t2,temp,thot,tcold,tmid,phi,phip,phim,invphip,omegat,uwxr,uwxl,uwxb,uwxt,uwyr,uwyl,uwyb,uwyt,cs2,dt,fbx,fby,sigt,sigtt,tauf,nu,q_o,lq,xo,w)
...
for x = 1:nnx
    for y = 1 : nny
        ...
        elseif (solidnode[in] == 3) #Top wall
            ...
            LAM21 = (-1.0 + ((2.0 * cs2 * LAM) / (rwt * nu * wf6))) / (1.0 + ((2.0 * cs2 * LAM)/(rwt * nu * wf6)))

            C11t = (-(cs2 / (wf5 * nu)) * LAM - (fs5 - fs6) - (0.5 * fby * dt * (fs1 - fs3 + fs5 - fs6)) / rwt) / (1.0 - (0.5 * fby * dt / rwt))
            C01t = - 0.5 * fby * dt - (fs2 + fs5 + fs6) 
            C21t = (-(cs2 * fby) / wf6 - ((cs2 * LAM) / (rwt * nu * wf6)) * fbx * dt - (fs5 + fs6) - ((2.0 * cs2 * LAM) / (rwt * nu * wf6)) * (fs1 - fs3 + fs5 - fs6)) / (1.0 + ((2.0 * cs2 * LAM)/(rwt * nu * wf6)))

            fs4 = (1.0 / (LAM21 - 1.0)) * (-C11t + C01t - 2.0 * C21t - LAM21 * C11t - LAM21 * C01t)
            fs7 = (1.0 / (LAM21 - 1.0)) * (LAM21 * C11t + C21t)
            fs8 = (1.0 / (LAM21 - 1.0)) * (C11t + C21t)
