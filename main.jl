...
@timeit to "Setup" begin 
    ...
    # Choose which approach to utilize: Choose 1, 2 or 3 for those approaches respectively

    appchoose = 1 

    if appchoose == 1 
        include("stream_app_1.jl")
    elseif appchoose == 2 
        include("stream_app_2.jl")
    elseif appchoose == 3 
        include("stream_app_3.jl")
    end
    ...
    #Capilary Number
    Ca = 10.0
    #Surface tension at "tmid"
    sigma = mu * Uo / Ca
    #Thermocapillarity (Normal Fluids)
    M1 = -10.0^(-1.0) 
    #Thermocapillarity (Self-Rewetting Fluids)
    M2 = 0.0
    #Surface Tension sensitivity Coeffecents. 
    sigt = M1 * sigma / dT
    sigtt = M2 * sigma / (dT * dT)
    #Gaussian Profile Width (as percentage of cavity width)
    lq = alx * 0.5
    #Heat Flux Magnitude
    Q = 0.02
    ...
    println("K= ",K)
    println("q_o= ",q_o)
    println("Approach Chosen: ",appchoose)
    ...
    name = joinpath("RA_$Ra _Q_$Q _M1_$M1 _M2_$M2 _Lq_$lq data_$time")
    mkpath(name)
    cd(name)
    l = 1
    ...
    \nQ= $Q M1= $M1 M2= $M2 sigma= $sigma sigt= $sigt sigtt= $sigtt dT= $dT K= $K q_o= $q_o Lq= $lq")
    close(aparam)
end 
...
@timeit to "Main Loop" begin
    for n= 1 : nsteps
        ...
        @timeit to "stream" begin
            stream(nnx,nny,nnode,ndir,vpi,solidnode,solidstr,f1,f2,t1,t2,temp,thot,tcold,tmid,phi,phip,phim,invphip,omegat,uwxr,uwxl,uwxb,uwxt,uwyr,uwyl,uwyb,uwyt,cs2,dt,fbx,fby,sigt,sigtt,tauf,nu,q_o,lq,xo,w4)
        end
